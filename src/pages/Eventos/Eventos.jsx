...
function Eventos() {
  ...
  const data = [
    ["Eventos", "", ""],
    ["Total de Eventos", qnt, 50],
  ];

  const options = {
    title: "Eventos de Deputados",
    chartArea: { width: "50%" },
    colors: ["#006600", ""],
    hAxis: {
  ...
  const pesquisar = async () => {
    try {
      await apiDeputados.get('https://dadosabertos.camara.leg.br/api/v2/deputados/' + dataid + '/eventos?ordem=ASC&ordenarPor=dataHoraInicio').then(resultado => {
